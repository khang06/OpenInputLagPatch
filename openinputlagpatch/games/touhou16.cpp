...
void th16_install_patches() {
	{
		...
		patch_bytes((void*)0x0045AC9D, patch, sizeof(patch));
	}
	{
		// Force fast input latency mode
		BYTE patch[] = { 0xEB };
		patch_bytes((void*)0x00459F72, patch, sizeof(patch)); // Skip over automatic
		patch_bytes((void*)0x00459F87, patch, sizeof(patch)); // Skip over normal
	}
	{
		// Hook window update
		patch_call((void*)0x00459FA3, th16_window_update_hook);
	}
	if (Config::D3D9Ex) {
