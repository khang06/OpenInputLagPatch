...
void th143_install_patches() {
	{
		...
		patch_bytes((void*)0x0046F02E, patch, sizeof(patch));
	}
	{
		// Force fast input latency mode
		BYTE patch[] = { 0xEB };
		patch_bytes((void*)0x0046E2E6, patch, sizeof(patch)); // Skip over automatic
		patch_bytes((void*)0x0046E304, patch, sizeof(patch)); // Skip over normal
	}
	{
		// Hook window update
		patch_call((void*)0x0046E317, th143_window_update_hook);
	}
	if (Config::D3D9Ex) {
